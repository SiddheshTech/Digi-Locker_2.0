...
export async function signPayloadHash(payloadHash) {
    const provider = getProvider();
    const signer = getSigner(provider);
    if (!signer) return '0xMOCK_SERVER_SIG_' + payloadHash.substring(0, 16);
    // Remove 0x prefix if present for signature
    const h = payloadHash.replace(/^0x/, '');
    return await signer.signMessage(Buffer.from(h, 'hex'));
}

/** Issue credential on-chain (or mock) */
export async function issueOnChain(payloadHash, issuerId) {
    const provider = getProvider();
    const signer = getSigner(provider);
    ...
    const bytes32 = payloadHash.startsWith('0x') ? payloadHash : '0x' + payloadHash.padStart(64, '0');
...
export async function revokeOnChain(payloadHash, reason) {
    const provider = getProvider();
    const signer = getSigner(provider);
    const contract = getContract(signer);
...
export async function queryCredentialOnChain(payloadHash) {
    const provider = getProvider();
    const addr = process.env.CONTRACT_ADDRESS;
    ...
    try {
        ...
        const bytes32 = payloadHash.startsWith('0x') ? payloadHash : '0x' + payloadHash.replace(/^0x/, '').padStart(64, '0');
