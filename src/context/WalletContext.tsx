 ...
 * Global MetaMask wallet state — connect, disconnect, auto-reconnect on refresh
 */
...
export function WalletProvider({ children }: { children: ReactNode }) {
    ...
    const connect = useCallback(async () => {
        if (!window.ethereum) {
            setState(s => ({ ...s, error: 'MetaMask not found. Please install the MetaMask browser extension.' }));
            return;
        }
        ...
        try {
            ...
            setState(s => ({ ...s, isConnecting: false, error: (err as Error).message || 'Connection rejected' }));
        }
    }, [updateState]);
    ...
    const switchToHardhat = useCallback(async () => {
        try {
            ...
            console.error('Switch to Hardhat failed:', err);
        }
    }, []);
